    ...
    def create_sale_orders(self):
        ...
        if session_product_id:
            for student in self.student_ids:
                order_id = self.env["sale.order"].create({"partner_id":student.id,"session_id":self.session.id,"order_line":[(0,0,{"product_id": session_product_id.id,"price_unit": self.session_id.total_price})] })